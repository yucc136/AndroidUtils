...
public class ImageUtil {
	...
	public static Bitmap rotate(Bitmap origin, int degrees) {
		if (degrees != 0 && origin != null && !origin.isRecycled()) {
			Matrix m = new Matrix();
			m.setRotate(degrees,
					(float) origin.getWidth() / 2, (float) origin.getHeight() / 2);
			try {
				Bitmap b2 = Bitmap.createBitmap(
				origin, 0, 0, origin.getWidth(), origin.getHeight(), m, true);

				if (origin != b2) {
					origin.recycle(); // Android开发网再次提示Bitmap操作完应该显示的释放
					origin = b2;
				}
	...
	public static Drawable geRoundDrawableFromUrl(Context context, String url,
	 ...
	 * @return
	 */
	public static BitmapDrawable toRoundCorner(Context context,
	...
	public static String getImageName() {
		...
		return date.toGMTString() + Math.random() * 1000 + ""
				+ date.getSeconds();
	}
}